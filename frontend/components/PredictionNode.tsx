...
export function PredictionNode({ asset }: { asset: Asset }) {
  ...
  return (
    ...
    >
      ...
      <div 
        className={`relative flex items-center gap-2.5 px-4 py-3 glass-1 shape-facet-sm transition-all duration-[var(--dur-hover)] ease-glide ${isHovered ? `scale-110 z-20 ${t.borderClass}` : `hover:bg-white/10 border-white/5`}`}
      >
        {/* Glow */}
      ...
      <GlassCard tier={3} shape="shape-ledger" className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-72 p-5 transition-all duration-[var(--dur-hover)] ease-glide origin-bottom pointer-events-none depth-bevel ${isHovered ? 'opacity-100 scale-100 z-[100] translate-y-0' : 'opacity-0 scale-95 z-0 translate-y-4'}`}>
        ...
        <div className="relative z-10">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className={`text-[9px] font-bold px-3 py-3 shape-facet-sm uppercase tracking-widest border ${t.textClass} border-current/30 shadow-[0_0_10px_currentColor] bg-currentColor/10`}>
                    {t.label}
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-5">
                <div className="bg-surface/40 rounded-sm p-2.5 border border-white/5">
                    ...
                    <span className="text-sm font-mono font-bold text-text">{confidence != null ? `${confidence.toFixed(1)}%` : "â€”"}</span>
                </div>
                <div className="bg-surface/40 rounded-sm p-2.5 border border-white/5">
                    <span className="text-[9px] text-text-muted/80 uppercase tracking-widest font-mono block mb-1">Price</span>
                    <span className="text-sm font-mono font-bold text-text capitalize">{price != null ? formatPrice(price) : "â€”"}</span>
                </div>
                <div className="bg-surface/40 rounded-sm p-2.5 border border-white/5">
                    ...
                    <span className={`text-sm font-mono font-bold ${asset.rsi_14 != null && asset.rsi_14 > 70 ? 'text-danger' : asset.rsi_14 != null && asset.rsi_14 < 30 ? 'text-success' : 'text-text'}`}>
                        {asset.rsi_14 != null ? asset.rsi_14.toFixed(1) : "â€”"}
                    </span>
                </div>
                <div className="bg-surface/40 rounded-sm p-2.5 border border-white/5">
                    ...
                    <span className={`text-sm font-mono font-bold ${asset.macd != null && asset.macd > 0 ? 'text-success' : 'text-danger'}`}>
                        {asset.macd != null ? asset.macd.toFixed(4) : "â€”"}
                    </span>
                </div>
            </div>
