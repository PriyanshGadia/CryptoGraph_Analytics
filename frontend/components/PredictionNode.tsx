...
export function PredictionNode({ asset }: { asset: Asset }) {
  ...
  const statusConfig: Record<string, { bg: string, text: string, shadow: string, label: string, borderAccent: string }> = {
    strong_up: { bg: "bg-success", text: "text-success", shadow: "shadow-[0_0_20px_rgba(34,197,94,0.6)]", label: "STRONG BUY", borderAccent: "border-success/50 shadow-[inset_0_0_15px_rgba(34,197,94,0.2)]" },
    up: { bg: "bg-success/80", text: "text-success", shadow: "shadow-[0_0_15px_rgba(34,197,94,0.4)]", label: "BUY", borderAccent: "border-success/40 shadow-[inset_0_0_10px_rgba(34,197,94,0.1)]" },
    down: { bg: "bg-danger/80", text: "text-danger", shadow: "shadow-[0_0_15px_rgba(239,68,68,0.4)]", label: "SELL", borderAccent: "border-danger/40 shadow-[inset_0_0_10px_rgba(239,68,68,0.1)]" },
    strong_down: { bg: "bg-danger", text: "text-danger", shadow: "shadow-[0_0_20px_rgba(239,68,68,0.6)]", label: "STRONG SELL", borderAccent: "border-danger/50 shadow-[inset_0_0_15px_rgba(239,68,68,0.2)]" },
    neutral: { bg: "bg-text-muted/60", text: "text-text", shadow: "shadow-[0_0_10px_rgba(148,163,184,0.2)]", label: "NEUTRAL", borderAccent: "border-white/10" },
  };
  ...
  return (
    ...
    >
      ...
      <div 
        className={`relative flex items-center gap-2.5 px-4 py-2 glass transition-all duration-500 rounded-full ${isHovered ? `scale-110 z-20 ${c.borderAccent}` : `hover:bg-white/10 border-white/5`}`}
      >
        {/* Glow */}
        <div className={`absolute inset-0 transition-opacity duration-500 opacity-20 ${c.bg} blur-md rounded-full -z-10 ${isHovered ? 'opacity-40 scale-125' : ''}`} />
        
        {/* Crypto Icon */}
        <div className="w-6 h-6 rounded-crypto bg-surface/50 flex items-center justify-center overflow-hidden border border-white/10 p-0.5 shadow-inner">
      ...
      <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-72 rounded-crypto-lg glass-panel p-5 transition-all duration-300 origin-bottom pointer-events-none shadow-2xl ${isHovered ? 'opacity-100 scale-100 z-[100] translate-y-0' : 'opacity-0 scale-95 z-0 translate-y-4'}`}>
        
        {/* Glow behind popup */}
        <div className={`absolute inset-0 rounded-crypto-lg opacity-10 blur-xl ${c.bg}`} />
        ...
        <div className="relative z-10">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className={`text-[9px] font-bold px-2 py-1 rounded-sm uppercase tracking-widest border ${c.text} border-current/30 shadow-[0_0_10px_currentColor] bg-currentColor/10`}>
                    {c.label}
                </div>
            </div>

            <div className="grid grid-cols-2 gap-3 mt-5">
                <div className="bg-surface/40 rounded-crypto p-2.5 border border-white/5">
