...
export function TableSkeleton({ rows = 10 }: { rows?: number }) {
  return (
    <div className="glass-flat shape-ledger overflow-hidden border-white/5">
...
export function ChartSkeleton({ height = 280 }: { height?: number }) {
  return (
    <div className="glass-flat shape-ledger p-8 relative overflow-hidden" style={{ height }}>
      <div className="h-5 w-64 bg-[rgba(var(--text),0.06)] shape-facet-sm animate-pulse mb-6"/>
      <div className="h-full bg-[rgba(var(--text),0.06)] shape-facet animate-pulse flex items-end px-6 gap-2 pb-6">
        {[...Array(12)].map((_,i) => (
