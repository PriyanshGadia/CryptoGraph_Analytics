  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] space-y-6">
        ...
        <button onClick={() => mutate()} className="flex items-center gap-2 px-6 py-3 glass bg-white/5 hover:bg-white/10 transition-all rounded-crypto-sm text-text border border-white/10 shadow-inner hover:shadow-[0_0_15px_rgba(255,255,255,0.1)] font-bold uppercase tracking-widest text-xs">
    ...
    <div className="space-y-8 pt-8 max-w-[1600px] mx-auto relative">
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-4">
        <div>
          <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-text via-text/80 to-text-muted flex items-center gap-4 tracking-tight">
            <div className="p-3 glass bg-danger/10 rounded-crypto shadow-inner shadow-danger/20">
      ...
      {isLoading || !data ? (
        <div className="space-y-8 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">{Array.from({length: 4}).map((_, i) => <Skeleton key={i} className="h-36 rounded-crypto" />)}</div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8"><Skeleton className="h-[400px] rounded-crypto" /><Skeleton className="h-[400px] rounded-crypto" /></div>
        </div>
      ) : (
        <div className="relative z-10 space-y-8">
          {/* Regime Banner */}
          <GlassCard asymmetric="lg" className={`p-0 overflow-hidden relative group border ${
            ...
            <div className="relative p-8 flex items-center gap-8">
                <div className={`p-5 rounded-crypto glass ${
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <GlassCard key={i} asymmetric="sm" className="p-6 flex items-center gap-4 group hover:bg-white/[0.02] transition-colors border border-white/10 hover:border-white/20 h-32">
            ...
            <GlassCard asymmetric="md" className="p-8">
                ...
                <div className="h-[300px] w-full relative">
                  {predDist.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-full text-text-muted font-mono border border-white/5 rounded-crypto bg-surface/30">Awaiting tensor computation...</div>
                  )}
                </div>
            </GlassCard>
          </div>
              ...
              <div className="p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {Object.entries(data.correlation_clusters).slice(0, 8).map(([cluster, assets]) => (
                    <div key={cluster} className="p-5 glass bg-surface/50 rounded-crypto border border-white/10 hover:border-accent/30 transition-colors group hover:shadow-[0_0_20px_rgba(var(--accent),0.1)]">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {assets.map((sym: string) => (
                          <span key={sym} className="px-3 py-1.5 rounded-crypto-sm text-xs font-mono font-bold bg-white/5 text-text-muted border border-white/10 group-hover:bg-accent/5 group-hover:text-text group-hover:border-accent/20 transition-colors">
                            {sym}
                          </span>
                        ))}
                      </div>
          ...
          <GlassCard asymmetric="md" className="p-0 overflow-hidden border border-white/10 relative">
            ...
            <div className="p-8 bg-black/20">
              {!data.risk_alerts || data.risk_alerts.length === 0 ? (
                <div className="flex items-center justify-center py-12 glass bg-success/5 border border-success/20 rounded-crypto shadow-inner shadow-success/10">
                    <div className="text-center">
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {data.risk_alerts.map((alert: RiskAlert, idx: number) => {
                    ...
                    return (
                      <li key={idx} className={`p-6 rounded-crypto glass border ${config.bg} ${config.border} ${config.shadow} relative overflow-hidden group`}>
                        ...
                        <div className="flex gap-4 items-start relative z-10">
                          ...
                          <div className="flex-1">
                            ...
                            {alert.recommendation && (
                              <div className="mt-4 p-3 bg-black/40 rounded-crypto-sm border border-white/5">
          ...
          {macro && (
            <div className="pt-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <GlassCard asymmetric="sm" className="p-6 flex items-center gap-6 group hover:border-white/20 transition-all hover:-translate-y-1">
                    <div className="p-4 rounded-crypto glass bg-accent/10 text-accent border border-accent/20 group-hover:scale-110 transition-transform"><Activity size={28} /></div>
                    <div>
                ...
                <GlassCard asymmetric="sm" className="p-6 flex items-center gap-6 group hover:border-white/20 transition-all hover:-translate-y-1">
                    <div className="p-4 rounded-crypto glass bg-warning/10 text-warning border border-warning/20 group-hover:scale-110 transition-transform"><TrendingUp size={28} /></div>
                    <div>
                ...
                <GlassCard asymmetric="sm" className="p-6 flex items-center gap-6 group hover:border-white/20 transition-all hover:-translate-y-1">
                    <div className="p-4 rounded-crypto glass bg-danger/10 text-danger border border-danger/20 group-hover:scale-110 transition-transform"><AlertTriangle size={28} /></div>
                    <div>
                  ...
                  <div className="h-[320px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={macro.history} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <Line type="monotone" dataKey="vix" name="VIX Level" stroke="rgb(234, 179, 8)" dot={false} strokeWidth={3} className="drop-shadow-[0_0_5px_rgba(234,179,8,0.5)]" />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </div>
                </GlassCard>
