...
db = SessionLocal()
try:
    preds = db.query(Prediction).order_by(Prediction.predicted_at.desc()).limit(5).all()
    for p in preds:
        asset = db.query(Asset).filter(Asset.id == p.asset_id).first()
        print(f"Asset: {asset.symbol if asset else p.asset_id}")
        print(f"  Predicted At: {p.predicted_at}")
        print(f"  Direction: {p.direction}")
        print(f"  Confidence: {p.confidence}")
        print(f"  shap_values: {p.shap_values}")
