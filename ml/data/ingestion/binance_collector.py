...
def main() -> None:
    ...
    for symbol in SYMBOLS:
        ...
        # Paginate CCXT data fully
        while True:
            retries = 0
            batch = []
            while retries < 5:
                try:
                    batch = exchange.fetch_ohlcv(pair, "1d", since=current_since, limit=1000)
                    break
        ...
        # Check if collected CCXT data is stale (last date is more than 2 days old)
        is_stale = False
        if ohlcv_data:
        ...
        if not ohlcv_data or is_stale:
            ...
            print(f"CCXT data for {symbol} is empty or stale. Falling back to yfinance...")
            try:
